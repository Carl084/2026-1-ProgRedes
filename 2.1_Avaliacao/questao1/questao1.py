...
def search_jpegs(dirctory): # Ler os arquivos no path e filtrar 
...
#path = r"C:\Users\carlo\Desktop\Carlos\imagens"

if not path: # Tratamento de erros
    exit("Caminho não encontrado.")
    
else:
    try:
        dirctory = os.path.abspath(path)
        photos = search_jpegs(dirctory) # Procurar as JPEGs no caminho
